...
public class ClockView extends View {
    ...
    private int boradlineLongWidth = 8;//表盘长线条长度
    private int boradlineShortLength = 20;//表盘短线条长度
    private int boradlineShortWidth = 4;//表盘短线条长度
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        hourLength = array.getDimension(R.styleable.ClockView_hour_length, dpToPx(80));
        hourHeight = array.getDimension(R.styleable.ClockView_hour_height, dpToPx(12));
        hourColor = array.getColor(R.styleable.ClockView_hour_color, Color.BLACK);

        minLength = array.getDimension(R.styleable.ClockView_min_length, dpToPx(100));
        minHeight = array.getDimension(R.styleable.ClockView_min_height, dpToPx(8));
        minColor = array.getColor(R.styleable.ClockView_min_color, Color.GREEN);

        secondLength = array.getDimension(R.styleable.ClockView_second_length, dpToPx(120));
        secondHeight = array.getDimension(R.styleable.ClockView_second_height, dpToPx(4));
    ...
    public void paintCenterPoint(Canvas canvas) {
        ...
        clockBoardPaint.setStyle(Paint.Style.FILL);
        canvas.drawCircle(0, 0, clockBoardRadiu / 10, clockBoardPaint);
    }
    ...
    public void paintSurroundLine(Canvas canvas) {
        for (int i = 0; i < 60; i++) {
            ...
            if (i % 5 == 0) {//长线条
                ...
                canvas.save();
                canvas.rotate(-90);
                canvas.drawText(textContent, clockBoardRadiu - boradlineLongLenght - w, h / 2, clockBoardPaint);
                canvas.restore();
            } else {//短线条
                clockBoardPaint.setColor(Color.BLACK);
                clockBoardPaint.setStyle(Paint.Style.FILL);
    ...
    private void paintHour(Canvas canvas) {
        clockBoardPaint.setColor(hourColor);
        clockBoardPaint.setStyle(Paint.Style.FILL);
        clockBoardPaint.setStrokeWidth(hourHeight);
        canvas.drawLine(-clockBoardRadiu / 6, -hourHeight / 2, hourLength, hourHeight / 2, clockBoardPaint);
    }

    private void paintMin(Canvas canvas) {
        clockBoardPaint.setColor(minColor);
        clockBoardPaint.setStyle(Paint.Style.FILL);
        clockBoardPaint.setStrokeWidth(minHeight);
        canvas.drawLine(-clockBoardRadiu / 6, -minHeight / 2, minLength, minHeight / 2, clockBoardPaint);
    }
    ...
    private void paintSecond(Canvas canvas) {
        ...
        canvas.drawLine(-clockBoardRadiu / 6, -secondHeight / 2, secondLength, secondHeight / 2, clockBoardPaint);
    }
